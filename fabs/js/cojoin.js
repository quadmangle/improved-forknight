...
function initCojoinForms() {
  ...
  function startEncryptionProgress() {
    ...
    return {
      update(pct) { bar.style.width = pct + "%"; },
      finish() { overlay.remove(); }
    };
  }
   ...
   * @param {object} progress Optional progress helper.
   */
  async function sendToCloudflareWorker(data, endpoint, progress) {
    console.log("Data is clean. Encrypting and sending to Cloudflare worker...");
    try {
      // Retrieve managed key and short-lived token from the server
      if (progress) progress.update(30);
      ...
      const workerName = atob('T3BzV29ya2Vy');
      const payloadData = { ...data, worker: workerName };
      if (progress) progress.update(60);
      const encrypted = await crypto.subtle.encrypt(
        { name: 'AES-GCM', iv },
        aesKey,
        encoder.encode(JSON.stringify(payloadData))
      );
